...
# ---------------------------------------------------------------------------
# Persistent model cache helpers
# ---------------------------------------------------------------------------


def _persistent_pipeline_models_enabled() -> bool:
    return os.getenv("LTX_KEEP_PIPELINE_MODELS", "").lower() in {"1", "true", "yes", "on"}


def _module_on_meta(model: torch.nn.Module) -> bool:
    return any(param.device.type == "meta" for param in model.parameters()) or any(
        buffer.device.type == "meta" for buffer in model.buffers()
    )


@contextmanager
def _maybe_cached_model(owner: object, attr: str, build: Callable[[], _M]) -> Iterator[_M]:
    """Yield a model, optionally keeping it on the block for later calls."""
    if not _persistent_pipeline_models_enabled():
        with gpu_model(build()) as model:
            yield model
        return

    model = getattr(owner, attr, None)
    if model is None or _module_on_meta(model):
        model = build()
        setattr(owner, attr, model)
    yield model


def _maybe_cached_iter(owner: object, attr: str, build: Callable[[], _M], fn: Callable[[_M], Iterator[T]]) -> Iterator[T]:
    if not _persistent_pipeline_models_enabled():
        model = build()
        return _cleanup_iter(fn(model), model)
...
class DiffusionStage:
    ...
    def _transformer_ctx(self, **kwargs: object) -> AbstractContextManager:
        ...
        return _maybe_cached_model(self, "_cached_transformer", lambda: self._build_transformer(**kwargs))
...
class PromptEncoder:
    ...
    def _text_encoder_ctx(self) -> AbstractContextManager:
        ...
        return _maybe_cached_model(self, "_cached_text_encoder", self._build_text_encoder)
    ...
    def __call__(
        ...
        *,
        ...
        with _maybe_cached_model(self, "_cached_embeddings_processor", self._build_embeddings_processor) as embeddings_processor:
...
class ImageConditioner:
    ...
    def __call__(self, fn: Callable[[VideoEncoder], T]) -> T:
        ...
        with _maybe_cached_model(self, "_cached_encoder", self._build_encoder) as encoder:
            return fn(encoder)
    ...
    def __call__(self, latent: torch.Tensor) -> torch.Tensor:
        ...
        with (
            _maybe_cached_model(
                self,
                "_cached_encoder",
                lambda: self._encoder_builder.build(device=self._device, dtype=self._dtype).eval(),
            ) as encoder,
            _maybe_cached_model(
                self,
                "_cached_upsampler",
                lambda: self._upsampler_builder.build(device=self._device, dtype=self._dtype).eval(),
            ) as upsampler,
        ):
...
class VideoDecoder:
    ...
    def __call__(
        ...
        return _maybe_cached_iter(
            self,
            "_cached_decoder",
            lambda: self._decoder_builder.build(device=self._device, dtype=self._dtype).eval(),
            lambda decoder: decoder.decode_video(latent, tiling_config, generator),
        )
    ...
    def __call__(self, latent: torch.Tensor) -> Audio:
        ...
        with (
            ...
            _maybe_cached_model(
                self,
                "_cached_vocoder",
                lambda: self._vocoder_builder.build(device=self._device, dtype=self._dtype).eval(),
            ) as vocoder,
        ):
